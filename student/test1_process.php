...
if(isset($_POST['btn_confirm'])){
	...
	$result5 = mysqli_query($link,$sql5); //chechck whether new student
	if(mysqli_num_rows($result5)==0){ 
		...
		$sql1="SELECT * FROM test1_questionmgt_master WHERE finact=0";
		$result1 = mysqli_query($link,$sql1);
		while($row1=mysqli_fetch_array($result1)){
			
			$radio_questionnme1="radio_questionnme".$row1['test1_questionmgt_mas_key'];
			
			if(isset($_POST[$radio_questionnme1])){
				$sql2="SELECT * FROM  student_test01_details WHERE studenttestmas_key='$student_testmas_key'
																AND student_key='$ses_ukey'
																AND test1_questionmgt_mas_key='$row1[test1_questionmgt_mas_key]'
																AND test1questionmas_key='$_POST[$radio_questionnme1]'
																AND finact=0";
				$result2 = mysqli_query($link,$sql2);
				if(mysqli_num_rows($result2)==0){
					$sql8="INSERT INTO student_test01_details (finact,status,student_test01_details_key,studenttestmas_key,student_key,test1_questionmgt_mas_key,test1questionmas_key)
					                                   VALUES (0,'A',NULL,'$student_testmas_key','$ses_ukey','$row1[test1_questionmgt_mas_key]','$_POST[$radio_questionnme1]')";
		...
		while($row1=mysqli_fetch_array($result1)){
			...
			if(isset($_POST[$radio_questionnme1])){
				...
				if(mysqli_num_rows($result2)==0){
					...
					$sql2="UPDATE  student_test01_details SET test1questionmas_key='$_POST[$radio_questionnme1]' WHERE studenttestmas_key='$student_testmas_key'
																AND student_key='$ses_ukey'
																AND test1_questionmgt_mas_key='$row1[test1_questionmgt_mas_key]'
																AND finact=0";
...
<html  class="bootstrap-admin-vertical-centered">
    ...
    <body class="bc" style="background-position:center;
				...
				<section class="panel panel-primary panel-transparent">
					<div class="panel-body">
						<div class="row">
						  ...
						  <div class="col-lg-6">
							<h1 style="color:#ffffff;font-weight:bold" align="center">Career Interest Test</h1>
							<h3 style="color:#ffffff;font-weight:bold" align="center">Please select one option in each row</h3>
						  </div>
						  <div class="col-lg-3">
							<div id="google_translate_element"></div> <!-- goodle Translation element -->
						  </div>
					</div>
				</section>
			</div>
										...
										<table class="table table-hover table-bordered" style="background-color:#ffffff;font-weight:bold" width="100%">
												...
												<?php
													...
													while($row4=mysqli_fetch_array($result4)){
														...
														$radio_questionnme="radio_questionnme".$row4['test1_questionmgt_mas_key']; 
														
														
														echo "<tr>
																<td width='40%'>".$question_a_nme."</td>
																<td width='10%'><input type='radio' name='".$radio_questionnme."' value='".$row4['question_a_key']."' required></td>
																<td width='40%'>".$question_b_nme."</td>
																<td width='10%'><input type='radio' name='".$radio_questionnme."' value='".$row4['question_b_key']."' required></td>
		...
		<script type="text/javascript"> <!-- Laguage Translation by google translator -->
