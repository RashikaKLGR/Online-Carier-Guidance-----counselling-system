...
if(mysqli_num_rows($result11)==0){
	echo "<script>
			...
			window.location.href='examdetails_enterd2.php';
		</script>";
}
?>
<?php
...
if(isset($_GET['cs'])){
	if($noofselectcourse<=2){
		...
		$sql7="SELECT * FROM  student_newselected_course_details WHERE student_key='$ses_ukey' AND course_key='$_GET[cs]' AND student_testmas_key='$student_testmas_key' AND test_type='Test02' AND finact=0";
		...
		if(mysqli_num_rows($result7)==0){
			$sql8="INSERT INTO student_newselected_course_details (finact,status,student_newselected_course_details_key,student_key,course_key,student_testmas_key,test_type,coursebatch_detail_key,batch_nme)
															VALUES (0,'A',NULL,'$ses_ukey','$_GET[cs]','$student_testmas_key','Test02','$student_batchs_key','$student_batchs_nme')";
			if(mysqli_query($link,$sql8)){
				echo "<script>
					alert('Successfully Select Course')
					window.location.href='test2_process3.php';
				</script>";
			}
		}
		else{
				...
				<section class="panel panel-primary panel-transparent">
					<div class="panel-body">
						<div class="row">
						  ...
						  <div class="col-lg-6">
							<h1 style="color:#ffffff;font-weight:bold" align="center">Career Key Test - Select Course</h1>
						  </div>
						  <div class="col-lg-3">
							 <a href="examdetails_enterd2.php"><button type="button" class="btn btn-lg btn-success btn-block">Education Qualification</button></a>
						  </div>
						</div>
					</div>
				</section>
				...
				<div class="col-lg-10">
					<section class="panel panel-primary panel-transparent">
						<div class="panel-body">
							...
							$s=1;
							$you_course=0;
							$sql1="SELECT * FROM  student_test02_result_details WHERE studenttestmas_key='$student_testmas_key' 
							...
							while($row1=mysqli_fetch_array($result1)){
								...
								if($s<=3){
									$sql4="SELECT * FROM test2_area_master WHERE test2_area_master_key='$row1[test2_areamas_key]' AND finact=0";
									$result4=mysqli_query($link,$sql4);
									while($row4=mysqli_fetch_array($result4)){
										$area_name=$row4['area_name'];
									}
									
									$sql2="SELECT * FROM course_master WHERE test2_areamas_key='$row1[test2_areamas_key]' AND finact=0";
									...
									while($row2=mysqli_fetch_array($result2)){
										...
										if($allresult_course<=$allresult_student){
											...
											echo '<tr>';
												...
												echo '</td>';
												echo '<td><a href="test2_process3.php?cs='.$row2['coursemas_key'].'"><button class="btn btn-sm btn-primary btn-block">Select</button></a></td>';
											echo '</tr>';
										}
									}
								}
						...
						<div class="panel-body">
							...
							<table class="table table-hover table-bordered" style="background-color:#79ed37;font-weight:bold" width="100%">
								...
								$sql9="SELECT * FROM student_newselected_course_details INNER JOIN course_master ON student_newselected_course_details.course_key=course_master.coursemas_key
																				  ...
																				  AND student_newselected_course_details.test_type='Test02' 
								...
								while($row9=mysqli_fetch_array($result9)){
									...
									echo '<tr>';
												...
												echo '</td>';
												echo '<td><a href="test2_process3.php?csd='.$row9['coursemas_key'].'"><button class="btn btn-sm btn-danger btn-block">Delete</button></a></td>';
									echo '</tr>';
								}
								?>
							</table>
