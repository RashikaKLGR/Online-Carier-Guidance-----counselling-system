...
if(isset($_POST['btn_add'])) {
    ...
    $nm1 = $_POST['txt_coursetype'];
    
	$sql1="SELECT * FROM  coursetype_master WHERE coursetype_nme='$nm1' AND finact=0";
	$result1 = mysqli_query($link,$sql1);
	if(mysqli_num_rows($result1)==0){
	
		$sql4="INSERT INTO coursetype_master (finact,status,coursetype_mas_key,coursetype_nme,act_person) 
								     VALUES(0,'A',NULL,'$nm1','$ses_ukey')";
		if(mysqli_query($link,$sql4)){
			
			$sql2="SELECT * FROM coursetype_master WHERE coursetype_nme='$nm1' AND finact=0";
			$result2 = mysqli_query($link,$sql2);
			while($row2=mysqli_fetch_array($result2)){
				$coursetype_mas_key=$row2['coursetype_mas_key'];
			}
			echo "<script>
				alert('Sucessfully Add Course Type.');
				window.location.href='cd_coursetypemenu.php';
			</script>";
		}
		else{
		...
		echo "<script>
				alert('Sorry,Dupilcate Course Type.');
			</script>";
	}
}
							...
							<div class="panel-body">
								<form role="form" method="post" name="f1">
									...
									<div class="form-group">
                                      ...
                                      <input type="text" class="form-control input-sm" name="txt_coursetype" placeholder="Please Enter Course Type">
									</div>
