...
if(isset($_POST['btn_add'])) {
    ...
    $nm1 = $_POST['txt_careerpath'];
    
	$sql1="SELECT * FROM  job_master WHERE jobname='$nm1' AND finact=0";
	$result1 = mysqli_query($link,$sql1);
	if(mysqli_num_rows($result1)==0){
	
		$sql4="INSERT INTO job_master (finact,status,jobmas_key,jobname) 
								     VALUES(0,'A',NULL,'$nm1')";
		if(mysqli_query($link,$sql4)){
			
			
			echo "<script>
				alert('Sucessfully Add Career Path');
				window.location.href='add_careerpath.php';
			</script>";
		}
		else{
		...
		echo "<script>
				alert('Sorry,Dupilcate Career Path');
			</script>";
	}
}
							...
							<div class="panel-body">
								<form role="form" method="post" name="f1">
									...
									<div class="form-group">
                                      <label><font color="red">&lowast;</font>Career Path</label>
                                      <input type="text" class="form-control input-sm" name="txt_careerpath" placeholder="Please Enter Career path">
									</div>
