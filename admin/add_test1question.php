...
if(isset($_POST['btn_add'])) {
	...
	$sql1="SELECT * FROM  test1_question_master WHERE test1_areamas_key='$nm1' AND type='$nm2' AND question_name='$nm3' AND finact=0";
	$result1 = mysqli_query($link,$sql1);
	if(mysqli_num_rows($result1)==0){
	
		$sql4="INSERT INTO test1_question_master (finact,status,test1questionmas_key,test1_areamas_key,type,question_name,act_person) 
								     VALUES(0,'A',NULL,'$nm1','$nm2','$nm3','$ses_ukey')";
		if(mysqli_query($link,$sql4)){
			echo "<script>
				alert('Sucessfully Add Question.');
				window.location.href='add_test1question.php';
			</script>";
		}
		else{
...
<head>
    ...
    <title>Add Test 01 - Question</title>
									...
									<div class="form-group">
										...
										<select class="form-control input-sm" name="sele_area" required>
											...
											while($row1=mysqli_fetch_array($result1)){
												$option1 = $option1."<option value=$row1[test1_area_master_key]>$row1[area_name]</option>";			//Load Reagon Name
											}
											?>
